...
而且沒把握的時候會**說得出為什麼**：紙面反光、光線不足、離太遠、紙跟桌面同色、
四個角沒全部入鏡 —— 這些判斷依據在偵測時本來就算過了，只是以前沒講出來。
一句「沒把握」不可行動，使用者只能原地再拍一張一模一樣的照片。門檻都是拿合成的
「拍壞」樣本量出來的，寧可少講一句也不要對一張其實沒問題的照片亂給建議。
...
**即時取景**（v3.20.0）：不必先拍再看歪不歪 —— 開相機的當下，畫面上就疊著會跟著
文件走的邊框，抓到了變綠、抓不穩變黃並列出上面那幾條重拍建議，**對準了才按快門**。
偵測是同步的、會擋住 UI 執行緒，所以不是每幀都跑：一次跑完才排下一次、兩次之間至少
350ms、取樣先縮到 640px 寬。框用 EMA 收手抖，但鏡頭真的移開時直接跟上 ——
平滑的代價是延遲，而延遲比抖動更難看。按下快門會對**留下來的那一張**重新偵測一次
（取景時的結果是「上一張」的，手持之下兩張不會一樣），所以接著開「拉正」看到的框，
就是這張照片本身的框。**框穩了會自己拍**（v3.22.0）：連續 3 次角點平均位移不到
畫面的 1.2% 就按下去，底下有倒數與穩定度長條，不想要就關掉「⏱ 自動」。
穩定度量的是**平滑前**的角點（平滑後的本來就會趨近不動，拿它算等於在量自己的濾波器），
而且抓不準一律歸零 —— 低信心的框「連續三次都在同一個地方」多半是連續三次都抓錯同一個東西。
...
**標註**（方框 / 箭頭 / 螢光筆 / 手寫）：在圖上直接把話講清楚 ——「這一區要改」
「這欄請填」「這一行是重點」。手勢跟打碼一模一樣（拖一下新增、點一下移除），差別是標註
**疊在上面**、拿掉就真的乾淨，而打碼是把像素改掉、回不去。方框與箭頭存的是拖曳的
起點 → 終點而不是矩形，所以箭頭指哪邊是留得住的；線條帶一圈深色暈邊，紅箭頭壓在
紅色印章上也看得見。手寫存的是整條軌跡，走中點二次曲線畫出來，不是有稜有角的折線。

螢光筆刻意跟其他三種不一樣：它用 `multiply` 疊上去 —— 一般的半透明疊色會把黑字
一起洗淡，multiply 只讓亮的地方染色，**刷過去字還是讀得到**。它沒有暈邊、是平頭
（圓頭會多蓋到下一個字），拖歪 8° 以內會自動扶正，而且一律畫在其他標註底下 ——
它是底色，畫在箭頭之上會把紅箭頭染成橘的。
...
- **身分驗證有兩種模式**，由 `ENABLE_AUTH` 決定：
  - `False`（預設）＝ **單人模式**，以 `X-User-Id` header 識別使用者，**不驗證、可被偽造**。
    這個模式只適合**內網或單人使用**，請勿在沒有外層防護（反向代理 / VPN）的情況下對公網開放。
  - `True` ＝ **多使用者模式**，要登入拿 Token（PBKDF2 雜湊密碼、Fernet 加密 Token、有效 24 小時）。
- **開 `ENABLE_AUTH` 之前一定要先設 `ENCRYPTION_KEY`。** 預設值是公開的（就寫在
  `app/config.py` 裡），用它簽出來的 Token 任何人都能偽造 —— 所以這個組合會讓服務
  **直接拒絕啟動**，而不是只印一行警告。
- **加密金鑰**：SMTP 密碼以 `ENCRYPTION_KEY` 加密儲存，身分 Token 用的是從它推導出來的
  **另一把**（domain separation，見 `app/utils/crypto.py`）—— 拿到一把回推不出另一把。
  想單獨換掉 Token 金鑰（例如把所有人踢下線）就設 `AUTH_SECRET_KEY`。
- **註冊不是開放的**：`/api/auth/register` 只開放給**第一個**帳號，之後自動關閉。
  要新增帳號就在 `.env` 設 `REGISTRATION_TOKEN`，把碼給對方，加完拿掉。
  ⚠️ 服務對外之後如果遲遲沒人註冊，第一個搶到的人就是主人 —— 啟動時會把這個窗口
  還開著明確講出來，要更嚴就從一開始設好 `REGISTRATION_TOKEN`。
- **登入與註冊有限流**：每個來源 IP 五分鐘 10 次。這條限流**只認連線來源、不看任何
  header** —— 其他端點的限流是用 `X-User-Id` 當配額標籤，而那個 header 客戶端自己填，
  拿來擋猜密碼等於沒擋。手上有一整片 IP 的攻擊者擋不住，那要另外做帳號層的計數。
- **登入 Cookie**：`HttpOnly` 一直都有；`Secure` 預設跟著請求的 scheme 走，
  反向代理後面請明確設 `COOKIE_SECURE=true`。
