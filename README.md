...
這一塊本來只長在後端的 OpenCV 裡，離線版只有自由裁切。搬進裝置時沒有照抄：

* 後端 v5 跑七種策略再評分挑最好的，瀏覽器沒有那個預算，所以只留一條路 ——
  **梯度導向的 Hough 直線偵測**。理由是文件的邊本來就是直線，而且直線由整條邊
  投票決定，**角被手指擋住也不影響**（後端當初得補凸包運算才解決這件事）
* 一般 Hough 每個邊緣點要投 180 個角度；這裡利用「梯度方向垂直於邊」，
  每點只投自己的角度加減兩度，快兩個數量級而且峰值乾淨得多
* 評分則整套照搬 v5 的觀念，因為那是 22 個案例的基準測試磨出來的：
  凸性（不懲罰透視梯形）、逐邊梯度支持度、最弱邊乘法門控
* 梯度門檻取分位數而不是寫死，昏暗的照片才不會整個失效

透視校正走 WebGL —— canvas 2D 只做得到仿射，做不到投影變換。一支 fragment shader
逐像素反推來源座標，雙線性取樣是 GPU 順便給的。拿不到 WebGL 的裝置退回
「切成細格、每格用仿射近似」，格子夠小時差距小於一個像素。

輸出尺寸用 **Zhang–He 法從透視幾何反推文件的真實寬高比**，不然斜拍的 A4
拉正之後會系統性地被拉扁（影像上的邊長跟 1:√2 差很多）。

拉正是破壞性的（換掉原圖），所以留了「還原原圖」。
...
手寫的簽名存的是**筆畫的點而不是像素**，所以：

* 蓋進 PDF 時輸出的是向量路徑，放大不糊、列印跟原生文件一樣銳利，一枚只佔幾 KB
* 曲線在 canvas 與 PDF 兩邊算的是同一條線（PDF 沒有二次貝茲運算子，換算成三次），
  所以畫面上看到的就是蓋出來的

匯入的圖片印章走影像 XObject + SMask 軟遮罩，中間是真的透明 —— 蓋在文字上不會把字糊掉。
