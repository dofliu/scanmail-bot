...
| 檔案 | 行數 | 做什麼 |
|------|------|------|
| `image-local.js` | ~1500 | 縮放 / 轉檔 / 壓縮 / 拼接 / 旋轉 / 裁切 / 打碼 / 標註（方框 / 箭頭 / 螢光筆 / 手寫）/ 濾鏡 / 文字 / 格子取景（裁切框與取景都會跟著旋轉走：`rotateRect` / `rotateFit`；螢光筆走 `multiply` 且一律畫在其他標註底下）|
...
| `ocr-lite.js` | ~250 | 裝置端 OCR（tesseract.js LSTM，英數）：共用一個 worker、資產位置由 `SM_OCR_PATHS` 給（App 打包在 vendor/、網頁版走 CDN）；`crop()` 從全解析度那張裁一塊區域，太小就放大到認得動 |
