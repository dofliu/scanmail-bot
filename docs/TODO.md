...
| 項目 | 為什麼 | 規模 |
|------|------|------|
| **取景時的節流門檻要看真機** | 偵測間隔現在固定 350ms、取樣固定 640px 寬，是照「單張 100–300ms」估的。低階手機上可能還是會頓，高階手機則是浪費 —— 拿真實裝置量過再決定要不要改成看 `session.latest().ms` 自動調整 | 調參數 |
| **打碼 / 標註不跟著旋轉走** | 兩者都畫在「裁切之後」的畫面上，所以先框好再按旋轉，框會留在原本的相對位置、指到別的內容。裁切框與格子取景在 v3.10.0 / v3.16.0 已經各自解掉（`rotateRect` / `rotateFit`），這兩個是同一類問題還沒補。做法照抄 `rotateRect`：打碼是矩形可以直接用，標註要對兩個端點各做一次同樣的映射 | 一個晚上 |
...
- [ ] **M2**：框的穩定度判斷（連續 N 幀角點變動 < 門檻）、倒數、自動快門。
      M1 已經在跑，`session.latest()` 每次都帶著平滑前的 `rawCorners`，
      穩定度可以直接從連續幾次的角點位移算 —— 但**門檻要拿真實裝置的手持資料抓**，
      合成串流是完全不會抖的，在它上面調出來的數字沒有意義
...
- [ ] **M1b**：把 `drawTexts` 的**單一**文字物件改成陣列。引擎那一側早就吃陣列了，
      卡住的是介面：`studio.jsx` 的 `text` 是一個 state 物件、`textLayer` 固定包成
      `[text]`，所以一次只有一個文字圖層。要做的是圖層清單 + 新增 / 刪除 + 把面板上
      十幾個滑桿改成操作「目前選中的那一層」。跟 M1 拆開是因為它是純介面的工，
      跟箭頭 / 方框沒有相依，混在一起會讓一個晚上做不完
- [ ] **M2**：螢光筆 / 手寫（接續 M1b 的多圖層）
...
| 郵件模板 | 8 種文件類型預設 + 自訂 |
| 測試 | 281 pytest（278 passed + 3 skipped）＋ 404 個瀏覽器測試（圖片 86 + 介面 103 + 簽名 52 + 文件 51 + 掃描 34 + 取景 32 + 位址 25 + 頁面 21） |
