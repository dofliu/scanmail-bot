...
進度是有的（`onProgress` 逐頁回報），缺的是「停」。`StudioReader` 目前用一個
run token 把過期的結果丟掉（換了框、換了模式都是新的一輪），所以畫面不會跳出
舊答案，但底下那一輪還是跑到完 —— tesseract.js 沒有中斷的 API，真的要停就是
`terminate()` 掉 worker 再重開，代價是下一次要重新載 6MB。

**轉文字讀不到「一個字都沒有」與「引擎壞了」的差別** —— `StudioReader` 兩種都
顯示訊息，但前者是提示（換個範圍）後者是錯誤。目前分得開，只是文案接近。
...
### 2026/09/01 (v3.28.1 — 快取破壞參數重新活過來)

- **`index.html` 的 `?v=` 全部凍在 3.15.0**，落後十三個版本；`mobile/package.json`
  也是同一個數字。沒人發現，因為沒有東西在守，而 ETag revalidation 又默默補救 ——
  但那個參數存在的意義就是不要依賴 revalidation。
- **修的是機制，不是那二十個字串。** 新的 `scripts/sync_version.py` 把 `main.py`
  的版號（宣告過的唯一來源）同步進兩個檔案，可重複跑；兩條 pytest 守門測試
  在任一個落後時變紅，錯誤訊息直接寫該跑哪個指令。
- **守門是先證明會咬才算數的**：先把 `main.py` bump 到 3.28.1、不同步 ——
  兩條守門立刻紅（正是「安靜凍住」那個失敗模式，現場重演一次）——
  再跑同步腳本，轉綠。
- `build_mobile.py` 打包時把每個 `?v=` 再改寫成正在打包的版本當保險：
  就算來源凍住，App 也不會把過期的參數帶出門。另外 `?v=` 全部消失會直接拒絕
  打包 —— 「把機制拿掉」也是一種「永不過期」，但那不是修好。
- `docs/DAILY_ROUTINE.md` 收尾契約補上一句：改完版號跑 `sync_version.py`。
- 測試 +2（pytest 304 → 306）。

### 2026/09/01 (v3.28.0 — 圖片轉文字的入口)

- **引擎從 v3.26.0 就在了，缺的一直是這個入口。** 唯一走得到 OCR 的路是
  「掃描型 PDF」—— 手上一張拍好的發票想把金額挖出來，得先包成 PDF 再轉檔。
  新的 `StudioReader` 面板，從單張工具列的「🔤 轉文字」進去。
- **這是 studio 裡第一個唯讀面板**，而這件事決定了它的形狀：打碼與標註都是
  把資料交回 `patch()`、以「套用」結束，這一頁什麼都不改、以「複製」結束。
  有一條 pytest 守門測試釘住那個分支不呼叫 `patch()`。
- **最要緊的決定是「從哪一張裁」。** 畫面上那張是預覽縮圖 —— 在它上面框住發票
  的一行字只有幾十個像素，而縮圖丟掉的資訊放大也補不回來。OCR 那條路一律從
  `renderItem(item)`（不帶 `usePreview`）裁，並且有守門測試釘住「先算全解析度、
  才裁」的順序。
- **放大是先量過才留下的**（v3.26.0 學到的就是解析度才是關鍵）：新的
  `SMOcrLite.crop()` 把窄於 1000px 的裁切放大到那個寬度。444px、26px 字的裁切，
  放大與不放大讀到一模一樣 —— 沒有好處；275px、11px 字的裁切，不放大讀成
  `03.962.50`、放大讀成 `03,982.50`（逗號變句點、8 變 6）。**後者才是測試**，
  因為只有它分得出差別。
- 一次一個框（打碼可以疊很多塊，但「讀哪裡」只有一個答案），點一下取消選取；
  開進來就先讀整張 —— 按下按鈕本身就是需求，再讓人多按一次沒有意義。
  引擎載入自己回報進度，因為網頁版第一次會現載 6MB。
- 「只要數字」把它**實際的行為**印在下面：它會把每個字硬塞成最接近的白名單
  字元，不是把文字濾掉，所以是給「框一格金額」用的，不是拿來清乾淨一整頁。
  接的時候順手量到一件事：那個模式的 `confidence` 一律回 0，所以信心只在
  全字集模式顯示。
- 測試 +19（pytest 301 → 304、OCR 30 → 37、studio 130 → 139）。studio 那一支是**真的**驅動面板、
  跑 `mobile/www/vendor` 裡打包好的 wasm，整條路沒有替身。
- **三個寫測試踩到的坑，都值得記下來**：①第一版失敗不是 OCR 的問題 ——
  診斷印出「1 個 canvas、工具列是沒選圖那一組」，原來是**沒有選取圖片**，
  而單張工具列只有選取之後才存在。②接著點畫布正中央還是選不到 —— 前面
  九百多行測試改過版面與間距，單張圖不保證畫在中間（掃 3×3 才找到，
  而且會回報是哪一點成功）。③留著選取狀態會弄壞**下一個區段**，
  它要的是沒選圖那一組工具列。
- 既有一條守門測試斷言 `useDragBoxes` 剛好兩個呼叫端，第三個加進來就紅了 ——
  改成斷言它真正的用意：**定義只有一份**。
- Mutation 檢查：拿掉放大 2 條紅、拿掉夾範圍 1 條、拿掉白底 1 條
  （拉正之後的透明邊角如果不填白，送進 tesseract 的是黑底而不是白紙）。
